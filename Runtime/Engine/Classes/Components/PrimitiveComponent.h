...
USTRUCT()
struct FSecondTickFunction : public FTickFunction
{
	GENERATED_USTRUCT_BODY()

		class UPrimitiveComponent* Target;

	ENGINE_API virtual void ExecuteTick(
		float DeltaTime,
		ELevelTick TickType,
...
template<>
struct TStructOpsTypeTraits<FSecondTickFunction> : public TStructOpsTypeTraitsBase2<FSecondTickFunction>
{
	enum
	{
		WithCopy = false
...
public:
	...
	*/
	...
	/** If enabled, instance overrides the world gravity with CustomGravity */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = Physics)
	uint32 bUseCustomGravity : 1;

	/**
	* Overrides gravity for this instance if bUseCustomGravity is true and gravity is enabled
	* is applied per kg mass of the affected object
	*/
	UPROPERTY(EditAnywhere, Category = Physics, meta = (editcondition = "bUseCustomGravity"))
...
protected:
	...
	FSecondTickFunction SecondaryComponentTick;
...
public:
	...
	void SubstepTick(float DeltaTime, FBodyInstance* BI);

	void TickPostPhysics(float DeltaSeconds, ELevelTick TickType, FSecondTickFunction& ThisTickFunction);

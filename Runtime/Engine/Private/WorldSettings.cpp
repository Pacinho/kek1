...
FVector AWorldSettings::GetGravity() const
{
	if (!bWorldGravitySet)
	{
		// try to initialize cached value
		AWorldSettings* const MutableThis = const_cast<AWorldSettings*>(this);
		MutableThis->WorldGravity = bGlobalGravitySet ? GlobalGravity : UPhysicsSettings::Get()->DefaultGravity;	//allows us to override DefaultGravityZ
	}

	return WorldGravity;
}

void AWorldSettings::OnRep_WorldGravity()
{
...
void AWorldSettings::GetLifetimeReplicatedProps( TArray< FLifetimeProperty > & OutLifetimeProps ) const
{
	...
	DOREPLIFETIME( AWorldSettings, WorldGravity );
...
void AWorldSettings::Serialize( FArchive& Ar )
{
	...
	if (Ar.UE4Ver() < VER_UE4_ADD_OVERRIDE_GRAVITY_FLAG)
	{
		...
		if(GlobalGravity != FVector::ZeroVector)
		{

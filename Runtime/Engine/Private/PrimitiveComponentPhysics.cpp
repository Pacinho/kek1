...
void UPrimitiveComponent::SetCustomGravity(FVector NewGravity)
{
	CustomGravity = NewGravity;
}

void UPrimitiveComponent::SubstepTick(float DeltaTime, FBodyInstance* BI)
{
	if (bUseCustomGravity)
	{
		BI->AddForce(CustomGravity, NAME_None, true);
	}
}

void UPrimitiveComponent::TickPostPhysics(float DeltaSeconds, ELevelTick TickType, FSecondTickFunction& ThisTickFunction)
{
	// Non-player update.

...
void UCharacterMovementComponent::PerformAirControlForPathFollowing(FVector Direction, float ZDiff)
{
	...
	if ( CharacterOwner && Velocity.Z < 0.f && (ZDiff < 0.f || GetGravity().Size() > 0.9f * GetWorld()->GetDefaultGravity().Size()))
	{
		if ( ZDiff < 0.f )
		{
			...
			if (IsMovingOnGround() || MovementMode == MOVE_Falling)
			{
				...
				if (!CurrentFloor.IsWalkableFloor())
				{
					if (!bSimGravityDisabled)
					{
						...
						{
							Velocity = NewFallVelocity(Velocity, GetGravity(), DeltaSeconds);
						}
					}
					SetMovementMode(MOVE_Falling);
				}
...
FVector UCharacterMovementComponent::GetGravity() const
{
	return Super::GetGravity() * GravityScale;
}
...
float UCharacterMovementComponent::GetMaxJumpHeight() const
{
	...
	if (Gravity.GetAbs() != FVector::ZeroVector)
	{
		return FMath::Square(JumpZVelocity) / (-2.f * Gravity.Size());
	}
	else
	{
		return 0.f;
...
void UCharacterMovementComponent::ApplyRootMotionToVelocity(float deltaTime)
{
	...
	if( bAppliedRootMotion && AppliedVelocityDelta.Z != 0.f && IsMovingOnGround() )
	{
		...
		else
		{
			...
			LiftoffBound = FMath::Max(GetGravity().Size() * deltaTime, SMALL_NUMBER);
		}
...
void UCharacterMovementComponent::PhysSwimming(float deltaTime, int32 Iterations)
{
	...
	if( !HasAnimRootMotion() && !CurrentRootMotion.HasOverrideVelocity() )
	{
		...
		CalcVelocity(deltaTime, Friction, true, GetMaxBrakingDeceleration());
		Velocity += GetGravity() * deltaTime * (1.f - NetBuoyancy);
	}
...
void UCharacterMovementComponent::PhysFalling(float deltaTime, int32 Iterations)
{
	...
	while( (remainingTime >= MIN_TICK_TIME) && (Iterations < MaxSimulationIterations) )
	{
		...
		// Apply gravity
		Velocity = NewFallVelocity(Velocity, GetGravity(), GravityTime);
			...
			else
			{
				...
				if (bHasLimitedAirControl)
				{
					// Compute VelocityNoAirControl
					{
						...
						VelocityNoAirControl = NewFallVelocity(VelocityNoAirControl, GetGravity(), GravityTime);
					}
...
void UCharacterMovementComponent::ApplyAccumulatedForces(float DeltaSeconds)
{
	if (PendingImpulseToApply.Z != 0.f || PendingForceToApply.Z != 0.f)
	{
		...
		if ( IsMovingOnGround() && (PendingImpulseToApply.Z + (PendingForceToApply.Z * DeltaSeconds) + (GetGravity().Size() * DeltaSeconds) > SMALL_NUMBER))
		{
			SetMovementMode(MOVE_Falling);
		}
	}

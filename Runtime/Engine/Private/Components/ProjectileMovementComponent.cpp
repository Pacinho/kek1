...
FVector UProjectileMovementComponent::GetGravity() const
{
	// TODO: apply buoyancy if in water
	return ShouldApplyGravity() ? Super::GetGravity() * ProjectileGravityScale : FVector::ZeroVector;
}
...
{
	return bForceSubStepping || (GetGravity().Z != 0.f) || (bIsHomingProjectile && HomingTargetComponent.IsValid());
}
